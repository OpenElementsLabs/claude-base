...
- **Thresholds** (from `${CLAUDE_PLUGIN_ROOT}/conventions/software-quality.md`): **backend ≥ 80%**, **frontend ≥ 70%**. For a fullstack project, evaluate each side against its own threshold.
...
These round out release-readiness and come from `${CLAUDE_PLUGIN_ROOT}/conventions/software-quality.md`. Report them, but a project can still be broadly healthy with warnings here:
